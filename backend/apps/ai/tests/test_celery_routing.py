...
def test_a_task_with_no_special_dependency_routes_to_the_default_worker():
    ...
    (``chunk_extraction``, the accounts email tasks) and future -- so
    adding a task without a route can never reproduce this bug silently."""
    assert _resolved_queue("apps.ai.tasks.chunk_extraction") == "default"
...
def test_every_routed_queue_has_a_worker_subscribed_to_it():
    for task_name in (
        ...
        "apps.ai.tasks.chunk_extraction",
    ):
...
def test_a_dispatched_task_is_consumed_by_a_worker_listening_on_its_queue():
    ...
    Dispatches ``chunk_extraction`` itself -- one of the two tasks this
    ...
    about Celery's own plumbing. A non-existent extraction id is deliberate: the
    task's own "deleted before the task ran" branch returns cleanly,
    ...
    with override_settings(
        ...
        try:
            with start_worker(celery_app, queues=["default"], perform_ping_check=False):
                result = chunk_extraction.delay(999_999)
