...
const EMPTY: FormState = {
  ...
  username: "", email: "",
  password: "", password2: "",
  course_id: "", college_id: "", dept_id: "",
};

// ── Validation ─────────────────────────────────────────────────────────────
/**
 * Full validation — used on submit to catch every problem.
 * password2 empty is an error here.
 */
function validateAll(f: FormState, role: Role): FieldErrors {
  ...
  if (!f.username.trim())    e.username    = "ID number is required.";
  ...
  if (role === "Adviser" && !f.college_id)
                             e.college_id = "Please select your college.";
...
/**
 * Live validation — used while typing.
 * Skips the "confirm password required" error while password2 is still empty,
 * so we don't flash an error before the user reaches that field.
 */
function validateLive(f: FormState, role: Role): FieldErrors {
  const e = validateAll(f, role);
  // Don't tell the user to "confirm password" until they start typing there
...
export default function SignupPage() {
  ...
  const filteredDepts = form.college_id ? deptsForCollege(form.college_id) : [];

  // Courses grouped by department (via <optgroup>)
  // We show ALL courses so the student can find their program regardless of dept filter
  const deptMap = new Map(DEPARTMENTS.map((d) => [d.id, d]));

  // Group courses by dept for optgroup rendering
  const courseGroups: { dept: string; courses: typeof COURSES }[] = [];
  const seen = new Set<number>();
  for (const c of COURSES) {
    if (!seen.has(c.dept_id)) {
      seen.add(c.dept_id);
      const dept = deptMap.get(c.dept_id);
      courseGroups.push({
        dept: dept ? `${dept.code} — ${dept.name}` : `Dept ${c.dept_id}`,
        courses: COURSES.filter((x) => x.dept_id === c.dept_id),
      });
    }
  }

  // ── Field change — instant validation ───────────────────────────────────
  const handleChange = (name: keyof FormState, value: string) => {
    const next = { ...form, [name]: value };
    setForm(next);

    // Mark touched immediately → error appears as soon as typing starts
    const nextTouched = { ...touched, [name]: true };
    setTouched(nextTouched);

    const liveErrs = validateLive(next, role);

    // Compute the updated errors object: keep existing errors, patch changed field
    const patch: FieldErrors = { [name]: liveErrs[name] };

    // When password changes, also re-evaluate confirm if user already typed there
    if (name === "password" && next.password2) {
      patch.password2 = liveErrs.password2;
    }

    setErrors((prev) => ({ ...prev, ...patch }));
  };

  // Blur marks the field touched and runs a full field validation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Mark all relevant fields touched so every error shows
    ...
    try {
      ...
      if (data && typeof data === "object") {
        ...
        for (const [key, val] of Object.entries(data)) {
          mapped[key as keyof FormState] = Array.isArray(val)
            ? (val[0] as string)
            : String(val);
        }
  ...
  // ── Role tab switch — reset form for that role, keep shared fields ────────
  const switchRole = (r: Role) => {
    setRole(r);
    setForm((prev) => ({ ...prev, course_id: "", college_id: "", dept_id: "" }));
    setErrors((prev) => {
      const next = { ...prev };
      delete next.course_id; delete next.college_id; delete next.dept_id;
      return next;
    });
    setTouched((prev) => {
      const next = { ...prev };
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4" noValidate>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className={`${cls("course_id")} appearance-none pr-9`}
                  >
                    <option value="">Select program</option>
                    {courseGroups.map(({ dept, courses }) => (
                  ...
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none text-[13px]">▾</span>
                </div>
                {errMsg("course_id")}
              </div>
            )}

            {/* ── College + Department (Adviser) ─────────────────── */}
            {role === "Adviser" && (
              <div className="grid grid-cols-2 gap-3">
                {/* College */}
                <div>
                  ...
                  <div className="relative">
                    <select
                      ...
                      className={`${cls("college_id")} appearance-none pr-9`}
                    >
            ...
            <div>
              ...
              {form.password && !errors.password && (
                <p className="text-[11px] text-green-600 mt-1">✓ Password looks good.</p>
              )}
              {errMsg("password")}
            </div>
            ...
            <button
              type="submit"
              disabled={!canSubmit || !terms}
              className="w-full py-3 rounded-lg text-[15px] font-semibold text-white transition-colors
                bg-[#6B0F12] hover:bg-[#7d1215]
                disabled:bg-gray-300 disabled:cursor-not-allowed mt-1"
            >
