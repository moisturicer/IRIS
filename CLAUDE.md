...
python manage.py load_corpus <dir> --dry-run             # bulk-load PDFs as Records (IR-313).
                                   ...
                                   # (publicly visible, so Ask IRIS retrieval can find them).
                                   # Idempotent by title within a category. Refuses with DEBUG off
                                   # unless --force. Drop --dry-run to write.
                                   # It creates the MANUSCRIPT PdfExtraction and queues extraction
...
python manage.py index_with_disclosure_bypass --dry-run  # index with ADR-015's disclosure
                                   ...
                                   # this is how the RAG path is exercised end to end meanwhile.
                                   # Encodes no policy and adds no field; honours the same spend
                                   # ceiling as backfill_embeddings. Refuses with DEBUG off unless
                                   # --force. The browser equivalent is the setting
                                   # AI_DISCLOSURE_BYPASS_FOR_DEVELOPMENT, which makes Django refuse
                                   # to start when DEBUG is off. Both are deleted by IR-250
