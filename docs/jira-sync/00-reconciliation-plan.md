...
| New | — | CREATE | Make the Docker stack buildable | Deployment | W1-W2 | **BLOCKER** | P0 | — | `D-01`. Both Compose files build `ai-gateway` from `./ai`. This originally read "which does not exist", true when the plan was drafted; since `7f73e97` the directory exists but Compose still requires `./ai/.env`, and ADR-012 rejects deploying the service |
