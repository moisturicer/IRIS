...
| FR-M3-02 | Semantic indexing | ADR-013 (§Decision "Retrieval scope," amended 2026-09-08), ADR-007, ADR-015 | `apps/ai/chunking/` (domain, strategies, context path, normalizer, regions, re-chunk diff) · `apps/ai/repositories.py` (ChunkSet persistence; atomic swap behind the `one_active_chunk_set_per_record` partial unique index; incremental re-chunk with vector carry-over and `deleted_at` tombstones) · `apps/ai/ingestion/` + `models/ingestion_job.py` (lifecycle transition table, hash-derived staleness, job idempotency key) · `apps/ai/ingestion/pipeline.py` + `ai.tasks.chunk_manuscript`, queued by `documents.tasks.extract_manuscript_text`, queued in turn by `RecordViewSet.perform_update` when a PATCH changes `abstract_file` — IR-89 A–H, rescoped to the manuscript only by IR-195. `PdfExtraction` now keys on `upload` *or* `record` (`resolved_record_id`); `documents.tasks.extract_pdf_text` (supplementary `UploadSlot` uploads) still extracts but no longer chunks — `ai.tasks.chunk_record_document` is retained but unqueued by anything | `apps/ai/chunking/tests/`, `apps/ai/ingestion/tests/` (incl. the `chunk_manuscript` cases), `apps/ai/tests/`, `apps/documents/tests/test_extraction_task.py` (supplementary path — asserts chunking does *not* fire), `apps/documents/tests/test_manuscript_extraction_task.py` (new), `apps/records/tests.py::ManuscriptExtractionTriggerTests` (new) | `pytest apps/ai apps/documents apps/records` green — a manuscript PATCH reaching an active chunk set with no manual step; a supplementary upload asserted *not* to reach one; zero writes on an unchanged document asserted with `CaptureQueriesContext`; one active set observed from a concurrent connection mid-swap; five thesis shapes (text-layer, scanned, table-heavy, long bibliography, mixed English/Cebuano) asserted for sane chunk counts, no empty chunk, and every chunk mapping to a page and a record | **PARTIAL** — the pipeline runs end to end on fixtures and no vectors are computed (IR-108); **the manual inspection IR-116 requires — fifty chunks from a real submission read by a person, and one real scan — has not been done, so the token ceiling and the front-matter decision are still the untested defaults**; backfilling chunk sets for records whose manuscript predates IR-195 is explicitly out of scope, not done |
