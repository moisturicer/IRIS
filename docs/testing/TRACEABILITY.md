...
| **FR-M5-01** | **Hierarchical submission workflow** | **ADR-002, ADR-003** | `reviews/services.py` — 11 guarded transitions, `RecordClearance`; `apps/reviews/clearance_state.py` derives `preserved`, `resubmission{}` and the peer summary in one place; `clearances[]` (with `status_label` and `preserved`) and `resubmission{}` on the record detail payload, peer clearance and waiting time on review-queue rows; `Record.resubmission_count` / `last_resubmitted_at` recorded by `resubmit_record` (IR-139) | `apps/reviews/test_clearance_state.py` (13, pure), `apps/reviews/test_clearance_payload.py` (12, DB) | `pytest apps/reviews` green — a clearance is `preserved` only after a genuine resubmission, a sequential decline names no declining office, and two offices provably get different queue rows for the same record | **PARTIAL** — the contribution is now observable through the API and tested; **`route[]` remains absent**, blocked on the declarative transition table (`IR-136`), whose own prerequisite **`IR-135` landed 2026-09-09**: the workflow vocabulary is now one `TextChoices` per concept in `core/enums.py`, 289 bare literals across 16 modules are gone, and `apps/tests/test_enum_vocabulary.py` fails the build if one comes back. `makemigrations` reports **no changes**, which is the evidence that nothing stored moved, and the reviewer UI that consumes this is `IR-143` |
