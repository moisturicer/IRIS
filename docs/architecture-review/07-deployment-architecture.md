...
| **Secrets** | ~~Hardcoded in both compose files, defaulted to the same value in `settings/base.py`, with a usable `SECRET_KEY` in `.env.example`~~ **Superseded by IR-154 (2026-09-08): fixed.** Compose interpolates `${DB_*:?}` from a repo-root `.env`; `base.py` has no defaults for them | Move DB credentials to `.env`; generate a real `SECRET_KEY`; ensure `.env` is gitignored (it is) |
