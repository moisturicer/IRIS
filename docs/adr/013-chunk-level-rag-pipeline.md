...
> **Amendment, 2026-09-21 — a chunk is measured in real `voyage-context-4` tokens.** This ADR made the chunk the retrievable unit and left its size to `ChunkingOptions.max_tokens`, without ever saying what a token was. The implementation answered by accident: `count_tokens` was `len(text.split())`, so every ceiling, floor and context-path budget in the pipeline was counted in whitespace *words* while being named in tokens. IR-243 measured the gap on a real 47-page submission — a chunk at the 512 ceiling held ~700 real BPE tokens — and deliberately left it, pending retrieval evidence that is now blocked behind IR-278's missing corpus.
>
> **IR-287 fixes the unit and holds the size.** `count_tokens` is the tokenizer `voyage-context-4` itself uses: a Qwen2 BPE vocabulary vendored at `apps/ai/chunking/tokenizer/voyage-context-4.json`, pinned by SHA-256, read through HuggingFace's `tokenizers` (pinned in `requirements/base.txt`). Nothing is downloaded at build or run time, and the domain still carries no vendor SDK — the determinism the whitespace count was chosen for is preserved by pinning both halves, and is now stronger, because the count agrees with what Voyage bills and with what the context window measures. `AI_CHUNK_MAX_TOKENS` moved 512 → 700 in the same change, the measured equivalent of the words it replaced, so **the unit changed and the chunk size did not**.

> **What the boundary evidence does and does not cover.** A words-to-tokens ratio is a property of the material, not a constant: measured with this tokenizer it is ~1.13 on clean flowing prose, ~1.37 on the real Docling-extracted submission 700 is calibrated on, and ~1.7 on dense technical markdown. `tests/test_token_unit_migration.py` therefore proves the **mechanism** — with the ceiling binding on a real section, boundaries are identical cut for cut once the ceiling is scaled by the document's own density — and bounds how far 700 sits from the equivalence band for prose of that density (518–673 there, so 700 is just above it). It does **not** demonstrate 700 itself, because demonstrating it needs a real Docling-extracted submission and no corpus exists until IR-278 lands. What is asserted unconditionally is the direction: the shipped ceiling never produces a *smaller* chunk than the word counter did, which is the silent re-chunking IR-243 forbade. What the right ceiling is for theses stays open for IR-133's recall@10 harness — the change is that the number now means what it says, and `token_count` remains excluded from the chunk-set hash so a future tokenizer upgrade does not stale the corpus. Tracked as [IR-287](https://citiris.atlassian.net/browse/IR-287).
...
**IR-287** (2026-09-21): implements the token-unit amendment above — the vendored tokenizer, the 512 → 700 ceiling that keeps chunk boundaries where they were, and the alignment of the vendor batching estimator with the chunker's count.
